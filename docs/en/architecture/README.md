...
Accepted design: [cumulative statistics and numeric contracts](cumulative-statistics.md). Tasks 4–9 are complete: catalogue, numeric L1–L4, spatial features, longitudinal statistics, [incremental storage](../development/cumulative-analysis.md) and the [L5–L6 analyst](../development/analyst-analysis.md). Task 10 follows. The authored chain below remains the task 3 prototype.

`src/apps/analysis/analyst/` reads aggregates through callable services and stores sessions/authored revisions beside numerical state. `services.py` prepares packets and accepts profiles; `retrieval.py` manages addressed reads/assessments; `comparison.py` excludes targets and selects earlier profiles. `schemas.py` defines metric reading and response contracts; `storage.py` persists versions/quotas; `reports.py` renders existing authored text; `cli.py` handles local exchange. Numerical revision and authored freshness remain separate.
