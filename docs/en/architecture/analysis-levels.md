...
| 2 | Selected L1 measurement windows | Behavior interpretation and alternatives | Current Codex |
| 3 | Accepted L2 analyses | Explain changes within a phase | Current Codex |
| 4 | Accepted L3 analyses | Personal match explanation | Current Codex |
| 5 | Accepted L4 analyses | Several conditional patterns and counterexamples | Current Codex |
| 6 | Accepted L5 analyses | Personal working portrait and development priorities | Current Codex |
...
Authored responses contain a synthesis, scope, what is not established, and claims marked observation/interpretation/hypothesis with evidence, alternatives and checks. Parents inherit child limitations. Missing lower analysis blocks preparation; revising a child makes dependent responses stale. Upper packets do not include full API responses. Seven levels do not require seven agents.

## Measurements and historical first-version contracts

The `Finding` structures and numerical screening below remain for measurements and historical experiments. They do not produce the current personal profile. The authored chain uses separately versioned `synthesis.py` analyses.
...
The first version had a reference graph, but separately stored model responses did not participate in synthesis. In `synthesis`, parents consume child interpretations together with their evidence and contradictions. Repeating a child's prose higher in the hierarchy does not make it an established fact.
